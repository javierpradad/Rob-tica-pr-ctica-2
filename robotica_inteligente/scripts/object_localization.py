...
def filter_img_objects(color_image, lower, upper):   
    ...
    # Save the coordinates of the object centroid in pixels, and a detected/not detected flag for roslaunch robotica_inteligente load_scene.launchthe object.
...
def depth_image_callback(depth_image_msg):
    ...
    if obt_detec and centroide_obj is not None:
        px, py = centroide_obj
        depth_obj = depth_image[py, px] / 1000.0

    if robot_detec and centroide_blue is not None:
        px, py = centroide_blue
        depth_blue = depth_image[py, px] / 1000.0
...
def ros_node():
    ...
    depth_image_sub = rospy.Subscriber('/camera/depth/image_raw', Image, depth_image_callback)# Corresponding callback:  depth_image_callback
    camera_info_sub = rospy.Subscriber('/camera/color/camera_info', CameraInfo, camera_info_callback)# Corresponding callback:  camera_info_callback
